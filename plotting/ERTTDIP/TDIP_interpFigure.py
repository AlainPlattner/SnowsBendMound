...
###### Import ERT and plot it as grayscale
import pybert as pb
...
a = pg.show(ip.pd, Mint*1000, cMin=4, cMax=20, ax=ax, label='M (mV/V)', cMap='inferno', colorBar=False, logScale=True)#, coverage=ip.coverage)
figname='interpretation_Mint_lines.pdf'
intfcol = 'silver'
...
if wannapick:
    ...
    if showcol:
        ...
        cbar = fig.colorbar(mappable, cax=cbar_ax, format='%g', label='chargeability [msec]') #ticks=[4, 6, 9, 13, 20]
        figname='interpretation_Mint_lines_colbar.pdf'
