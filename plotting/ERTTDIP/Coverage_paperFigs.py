...
a = pg.show(ip.pd, cov*100, ax=ax, cMap='viridis', cMin=0, cMax=100, logScale=False, colorBar=False) # pad=0.1, size='20%')
...
cbar = fig.colorbar(mappable, cax=cbar_ax, ticks=[0, 25, 50, 75, 100], format='%g', label='relative coverage [%]')
cbar.ax.minorticks_off()
    
plt.savefig('%s-coverage.pdf' %(figname))
